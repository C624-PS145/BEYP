...
const getFullPortfolioContext = async () => {
    ...
    const certificationsPromise = queryAndFormat(
        'SELECT id, category, name, issuer, year, link FROM certifications ORDER BY id DESC, name ASC',
        results => results
    );
    ...
    const educationPromise = (async () => {
        ...
        const educationIds = eduRows.map(edu => edu.id);

        const [publications] = await db.query(
            'SELECT education_id, title, authors, publisher, index_level AS `index`, year, link FROM publications WHERE education_id IN (?) ORDER BY year DESC',
            [educationIds]
        );
        const [achievements] = await db.query(
            'SELECT education_id, description, link FROM achievements WHERE education_id IN (?) ORDER BY id DESC',
            [educationIds]
        );

        return eduRows.map(edu => ({
          ...edu,
          publications: publications
            .filter(p => p.education_id === edu.id)
            .map(({ education_id, ...rest }) => rest),
          achievements: achievements
            .filter(a => a.education_id === edu.id)
            .map(({ education_id, ...rest }) => rest)
        }));
    })();
